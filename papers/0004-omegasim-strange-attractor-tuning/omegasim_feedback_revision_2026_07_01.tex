...
\[
  \sigma_\alpha(x;\theta)=\frac{1}{1+\exp(-\alpha(x-\theta))},
\]
where $\alpha>0$ is the appraisal steepness (sensitivity to deviations from
threshold $\theta$).  The amplitude $\beta>0$ is applied outside the unit
sigmoid as the saturation level (asymptotic maximum response).  The effective
response in the hive update replaces $S(\cdot)$ with
\[
  S_i(\cdot)\;\mapsto\; \beta_i\,\sigma_{\alpha_i}(w_i^\top x_k + b_i;\theta_i),
\]
giving each component $i$ its own threshold, steepness, and amplitude.
\end{definition}

\begin{observation}[Appraisal threshold as bifurcation coordinate]
The appraisal steepness $\alpha$ acts as a gain parameter: sufficiently low
$\alpha$ suppresses sensitivity to deviations from threshold, while high
$\alpha$ creates a switching-like regime with sharp transitions.  The threshold
$\theta$ shifts the operating point.  Together $(\alpha,\theta)$ define a
natural two-parameter sweep from low-gain response to thresholded multi-state
dynamics.  Whether an actual bifurcation occurs depends on the surrounding hive
update map and must be checked numerically in each experiment.
\end{observation}

\begin{justification}
For $\alpha\to 0$, the unit sigmoid approaches the constant $1/2$ and its
maximum derivative tends to zero.  For finite $\alpha$, the response
$\beta\sigma_\alpha$ has maximum derivative $\alpha\beta/4$ at the threshold.
For $\alpha\to\infty$, the response approaches a scaled Heaviside step.  Thus
$\alpha\beta/4$ is a local gain bound for the appraisal nonlinearity, not by
itself a proof of global contraction or bifurcation.
\end{justification}
...
\begin{designprinciple}[Cost-induced self-regulation]
If prediction cost is chosen to increase with a measured complexity proxy, such
as a local finite-time Lyapunov estimate or forecast-error budget, then highly
chaotic regimes incur higher costs, reducing effective gain and creating a
self-regulating feedback loop.  This is a proposed design mechanism for bounded
complexity; the strength and stability of the mechanism must be measured in the
A7 experiments rather than assumed analytically.
\end{designprinciple}
...
\begin{hypothesis}[Adaptive thresholds can create hysteresis]
When the adaptation rate $\eta_i$ is small but nonzero, the threshold
$\theta_{i,k}$ tracks the recent activation level with a finite relaxation time.
On upward parameter sweeps (increasing gain $\alpha$), the threshold may rise to
suppress over-activation; on downward sweeps, it may lower only after a delay.
This lag can produce hysteresis: the system's state at gain $\alpha$ on the
upward sweep may differ from its state at the same $\alpha$ on the downward
sweep.  The loop width should be reported empirically as a function of the
adaptation rate $\eta_i$ and the imposed sweep rate $\epsilon$.
\end{hypothesis}

\begin{justification}
Let $\bar\theta(\alpha)$ denote a quasi-steady threshold, when such a branch is
well defined.  Under a sweep $\alpha_k=\alpha_0+\epsilon k$, the adaptive update
is a first-order lag rather than instantaneous equilibration.  The sign of the
lag changes with sweep direction, which is the qualitative source of the
hysteresis test.  A universal scaling law such as $\eta_i/\epsilon$ is not
asserted here; the relevant scaling depends on the chosen update convention and
will be estimated in the proposed sweep.
\end{justification}
...
\subsection*{Replication Status (2026-07-16): Bounded Proxy Result}

\emph{This subsection records a measured result only. It does not strengthen the
strange-attractor claim or the semantic-grammar (Hyperseed feedback-loop) claim
of this note. Those claims remain conjectural and gated by the acceptance
criteria stated above; none of the six candidate-regime gates
(boundedness, refinement, perturbation/FTLE, surrogate, semantic, task) is
asserted as passed on the basis of the result below.}

\paragraph{Observed.} A repaired untouched-seed proxy replication
(OmegaSim frozen-CLA restart; proxy run pinned to \texttt{OmegaSim 18c7408} and
\texttt{chaoslang 974af31}) reproduced the specific parameter cell
$(\text{coupling}=0.60,\ \text{delay}=3,\ \text{roles}=8)$ in $4/5$ untouched
seeds --- i.e.\ four of five seeds yielded that cell. This $4/5$ figure and the
earlier $1/12$ figure have \emph{different denominators} and are not the same
quantity: $1/12$ is one promoted cell among twelve tested cells (a cell-selection
rate), whereas $4/5$ is four seed-wins among five seeds \emph{for that already
selected cell} (a per-cell seed-reproduction rate). The $4/5$ figure therefore
does \emph{not} supersede the $1/12$ figure; both stand, at different levels of
the pipeline.

\paragraph{Epistemic status.} \emph{Bounded proxy evidence; replication complete
for the proxy, formal claims blocked.} The $4/5$ seed-reproduction rate was
obtained under exact controls (matched controls were present, not absent);
replication of the bounded proxy result is complete. Nevertheless the
strange-attractor and semantic-grammar (Hyperseed feedback-loop) claims remain
\emph{blocked}, because the external Mackey--Glass / Lorenz--96 calibration
\emph{failed at the coding stage} and so the six candidate-regime gates
(boundedness, refinement, perturbation/FTLE, surrogate, semantic, task) cannot be
adjudicated against an external chaotic baseline. This datum is thus a completed,
exact-controlled proxy replication, but not confirmation of a strange-attractor
regime and not evidence for a Hyperseed semantic grammar; formal promotion awaits
a \emph{working} external calibration harness.

\paragraph{Provenance.} Recorded per Ben's instruction of 2026-07-16 to revise
Note 0004 \emph{only} to record the bounded proxy result, leaving the attractor
and semantic-grammar claims unstrengthened. Proxy lineage pins:
\texttt{OmegaSim 18c7408}, \texttt{chaoslang 974af31}; cell $(0.60,3,8)$
reproduced in $4/5$ untouched seeds under exact controls; cell-selection
rate $1/12$ (distinct denominator). Note: \texttt{c8e9e91} is a \emph{separate,
stricter held-out calibration path} and is \emph{not} the lineage of this proxy
run. Formal claims blocked pending a working external Mackey--Glass / Lorenz--96
calibration (prior attempt failed coding).